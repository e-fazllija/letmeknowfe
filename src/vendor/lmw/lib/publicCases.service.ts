...
// Helper: resolve publicCode by secret (via publicApi) then fetch canonical status
export async function getPublicReportStatusBySecret(secret: string) {
  // Try resolving via public API (tenant token endpoint)
  const info = await publicApi.getReportBySecret(secret);
  const code = (info as any)?.publicCode || (info as any)?.report?.publicCode || '';
  if (!code) throw new ApiError('Codice pratica non recuperabile dal segreto', 404 as any, {} as any);
  return getPublicReportStatus(code, secret);
}
