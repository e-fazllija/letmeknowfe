...
const DEV_TENANT_ID = (import.meta as any).env?.VITE_DEV_TENANT_ID;
...
async function http<T>(input: RequestInfo, init?: RequestInit): Promise<T> {
  ...
  // Solo in sviluppo, inviamo l'header x-tenant-id
  if (IS_DEV && DEV_TENANT_ID) headers['x-tenant-id'] = String(DEV_TENANT_ID);
