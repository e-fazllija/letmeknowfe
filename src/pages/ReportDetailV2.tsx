...
export default function ReportDetailV2() {
  ...
  const assigneeId: string | undefined = useMemo(
    () =>
      String(
        (report as any)?.internalUserId || (report as any)?.assigneeId || (report as any)?.assignedToUserId || ""
      ) || undefined,
    [report]
  );
  ...
  const {
    ...
    formState: { isSubmitting: statusBusy },
    reset: resetStatus,
    watch: watchStatus,
  } = useForm<StatusForm>({
    resolver: zodResolver(StatusSchema),
    defaultValues: { status: (report?.status as ReportStatus) ?? "OPEN" },
  });
  useEffect(() => {
    if (report?.status) resetStatus({ status: report.status as ReportStatus });
  }, [report?.status]);
  ...
  const { register: regPub, handleSubmit: submitPub, reset: resetPub, formState: { isSubmitting: pubBusy } } = useForm<MessageForm>({
    resolver: zodResolver(MessageSchema),
  ...
  useEffect(() => {
    if (!id || !canSeeNotes) return;
    (async () => {
      try {
        const res = await getMyNote(id);
        setMyNote((res?.body ?? "") as string);
        setMyNoteSavedAt(res?.updatedAt);
        setMyNoteError(null);
      } catch (e: any) {
        setMyNote("");
        setMyNoteSavedAt(undefined);
        setMyNoteError(e?.response?.data?.message || e?.message || null);
      }
    })();
  }, [id, canSeeNotes]);
  useEffect(() => {
    if (!id || !canSeeNotes || !canOperate) return;
    (async () => {
      try {
        setMyNoteSaving(true);
        const res = await putMyNote(id, debouncedNote || "");
        setMyNoteSavedAt(res?.updatedAt);
        setMyNoteError(null);
      } catch (e: any) {
        setMyNoteError(e?.response?.data?.message || e?.message || "Impossibile salvare la nota");
      } finally {
        setMyNoteSaving(false);
      }
    })();
  }, [debouncedNote, id, canOperate, canSeeNotes]);

  const referencedUserIds = useMemo(() => {
  const ids = new Set<string>();

  if (assigneeId) ids.add(String(assigneeId));
...
const userMap = useUserResolver(referencedUserIds);

const assigneeLabel: string = useMemo(() => {
    if (!assigneeId) return "-";
    if (meId && assigneeId === meId) return "me";
    const rpt = report as any;
    const reportEmail =
      rpt?.assignee?.email ||
      rpt?.assignedTo?.email ||
      rpt?.internalUser?.email ||
      rpt?.assigneeEmail ||
      rpt?.assignedToEmail ||
      null;
  ...
  const auditorsLabel: string = useMemo(() => {
    const r: any = report || {};
    const list: any[] = Array.isArray(r.auditors)
      ? r.auditors
      : Array.isArray(r.reportAuditors)
      ? r.reportAuditors
      : Array.isArray(r.auditorUsers)
      ? r.auditorUsers
      : [];
    const single = r?.auditorId || r?.auditor?.id || undefined;
    ...
    if (single) labels.add(String(single));
    const arr = Array.from(labels);
    return arr.length ? arr.join(", ") : "-";
  }, [report]);

  const reporterName: string | null = useMemo(() => {
    const priv = String((report as any)?.privacy || (report as any)?.privacyMode || "").toUpperCase();
    if (priv !== "CONFIDENZIALE" && priv !== "CONFIDENTIAL") return null;
    const n =
      (report as any)?.reporterName ||
      (report as any)?.reporter?.name ||
      (report as any)?.reporter?.fullName ||
      (report as any)?.reporterFullName ||
      (report as any)?.reporter?.displayName ||
      null;
  ...
  function buildSystemEvents(r: any) {
    ...
    for (const k of keys) {
      ...
      out.push({
        ...
        type: "SYSTEM",
        note: k.toUpperCase(),
        body: `${EVENT_LABELS[k] || k}: ${fmtDate(at)}`,
        createdAt: at,
      });
    }
    return out;
  ...
  function isSystemMessage(m: any) {
    ...
    const noteSys =
      note.startsWith("CASE_") ||
      ["CREATEDAT", "ASSIGNEDAT", "ACKNOWLEDGEAT", "INPROGRESSAT", "DUEAT", "DEADLINE", "CLOSEDAT", "FINALCLOSEDAT"].includes(note);
  ...
  // MOSTRA:
  // - se auditor: solo system
  // - altrimenti: tutti tranne PUBLIC (i PUBLIC stanno nella chat pubblica)
  const shownMessages = useMemo(() => {
    if (auditor) return combinedMessages.filter(isSystemMessage);
    return combinedMessages.filter((m: any) => {
      if (isSystemMessage(m)) return true;
      const vis = String(m?.visibility || "INTERNAL").toUpperCase();
      return vis !== "PUBLIC";
    });
  }, [auditor, combinedMessages]);

  // SOLO messaggi PUBLIC (per "Chat pubblica")
  const publicMessages = useMemo(() => {
    const arr = (Array.isArray(messages) ? messages : []).filter(
      (m: any) => String(m?.visibility || "").toUpperCase() === "PUBLIC"
    );
    const toTime = (x: any) => {
  ...
  // Auto-scroll chat pubblica alla ricezione di nuovi messaggi
  useEffect(() => {
    try {
  ...
  function renderMessageBody(m: any): string {
  const raw = String((m as any)?.body || "");

  // Se non è un messaggio di sistema, o se l’utente non è admin,
  // restituiamo il testo così com’è.
  if (!isSystemMessage(m) || !isAdmin) return raw;

  let out = raw;

  try {
    const map = userMap as any;
    if (map && typeof map === "object") {
      for (const key of Object.keys(map)) {
        const u = map[key];
        const label = u?.email || u?.displayName || u?.name;
        if (!label) continue;

        // sostituiamo tutte le occorrenze del userId dentro al body
        if (out.includes(key)) {
    ...
    // fallback: se c'è un errore, mostriamo comunque il raw
  }
  ...
  async function onStatusSubmit(data: StatusForm) {
    try {
      ...
      if (s === 400) setToast({ show: true, message: "Transizione non consentita", variant: "danger" });
      else if (s === 403) setToast({ show: true, message: "Operazione non consentita per il tuo ruolo", variant: "danger" });
      else setToast({ show: true, message: "Errore di connessione", variant: "danger" });
    }
  }
  ...
  async function onMessageSubmit(data: MessageForm) {
    try {
      ...
      const s = e?.response?.status;
      if (s === 403) setToast({ show: true, message: "Operazione non consentita per il tuo ruolo", variant: "danger" });
      else setToast({ show: true, message: "Errore di connessione", variant: "danger" });
    }
  }

  // Wrapper per messaggi PUBLIC dalla chat pubblica
  async function onPublicSubmit(data: MessageForm) {
    await onMessageSubmit({ body: data.body, visibility: "PUBLIC" } as MessageForm);
    try {
      resetPub({ body: "", visibility: "PUBLIC" });
    } catch {}
  }
  ...
  async function onAssignMeClick() {
    try {
      ...
      const s = e?.response?.status;
      if (s === 409) setToast({ show: true, message: "Già assegnato a un altro utente", variant: "danger" });
      else if (s === 403) setToast({ show: true, message: "Operazione non consentita per il tuo ruolo", variant: "danger" });
      else setToast({ show: true, message: "Errore di connessione", variant: "danger" });
    }
  }
  ...
  async function onTake() {
    try {
      ...
      setToast({
        show: true,
        message: (res as any)?.message === "ALREADY_IN_PROGRESS" ? "Già in carico" : "Caso preso in carico",
        variant: "success",
      });
      await loadReport({ silent: true });
    } catch (e: any) {
  ...
  function onFilesSelected(files: FileList | null) {
    ...
    if (next.length > MAX_FILES) {
      setToast({ show: true, message: `Limiti: max ${MAX_FILES} file`, variant: "danger" });
      return;
    }
    if (total > MAX_TOTAL_MB * 1024 * 1024 || next.some((f) => f.size > MAX_MB * 1024 * 1024)) {
      setToast({
        show: true,
        message: "Limiti: max 3 file, 10 MB ciascuno, 20 MB totali; tipi consentiti: png/jpeg/pdf/txt/mp3/wav.",
        variant: "danger",
      });
      return;
    }
    if (incoming.some((f) => !ALLOWED.includes(f.type || ""))) {
  ...
  async function savePendingAttachments() {
    ...
    try {
      ...
      for (const f of pendingFiles) {
        const items = await presignBatch([
          { fileName: f.name, mimeType: f.type || "application/octet-stream", sizeBytes: f.size },
        ]);
      ...
      const fin = (await finalizeBatch(toFinalize as any)) as any;
      ...
      if (accepted.length) {
        ...
        try {
          await attachToReport(id, toAttach as any);
          setToast({ show: true, message: `Allegati collegati: ${toAttach.length}`, variant: "success" });
          setPendingFiles([]);
        } catch {
  ...
  return (
    <div className="page-shell">
      <div className="container-fluid py-2">
        ...
        <Card.Header className="d-flex justify-content-between align-items-center">
          ...
          <div className="d-flex align-items-center" style={{ gap: 8 }}>
            {!auditor && !hasAssignee && (
              <Button size="sm" disabled={assignBusy} onClick={onAssignMeClick}>
                Assegna a me
              </Button>
            )}
            {!auditor && statusOpen && hasAssignee && (
              <Button size="sm" variant="dark" disabled={assignBusy || loadingTake} onClick={onTake}>
        ...
        <Card.Body>
          {/* Riga principale: Assegnato a + controlli assegnazione admin */}
          <div className="d-flex align-items-end flex-wrap" style={{ gap: 8 }}>
            <div className="mb-1">
              <strong>Assegnato a:</strong> {assigneeLabel}
            </div>
            ...
            {isAdmin && (
              <div className="d-flex align-items-end" style={{ gap: 8 }}>
                <Form.Select
                  ...
                  disabled={loadingUsers || assignBusy}
                  style={{ minWidth: 260 }}
                >
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      setAssignBusy(true);
                      await assign(id, assignUserId);
                      setToast({ show: true, message: "Assegnazione aggiornata", variant: "success" });
                      await loadReport({ silent: true });
                    } catch (e: any) {
                      const s = e?.response?.status;
                      if (s === 403) setToast({ show: true, message: "Operazione non consentita", variant: "danger" });
                      else if (s === 409) setToast({ show: true, message: "Già assegnato", variant: "danger" });
                      else setToast({ show: true, message: "Errore di connessione", variant: "danger" });
                    } finally {
                      setAssignBusy(false);
                    }
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      setAssignBusy(true);
                      await unassign(id);
                      setToast({ show: true, message: "Assegnazione rimossa", variant: "success" });
                      await loadReport({ silent: true });
                    } catch (e: any) {
                      const s = e?.response?.status;
                      if (s === 403) setToast({ show: true, message: "Operazione non consentita", variant: "danger" });
            ...
            <div className="mt-3">
              ...
              <div className="d-flex align-items-end" style={{ gap: 8, flexWrap: "nowrap" }}>
                ...
                <Button
                  size="sm"
                  variant="secondary"
                  disabled={!assignAuditorId || assignAuditorBusy}
                  onClick={async () => {
                    try {
                      setAssignAuditorBusy(true);
                      await assignAuditor(id, assignAuditorId);
                      setToast({ show: true, message: "Auditor assegnato", variant: "success" });
                      await loadReport({ silent: true });
                    } catch (e: any) {
                      const s = e?.response?.status;
                      if (s === 403) setToast({ show: true, message: "Operazione non consentita", variant: "danger" });
                      else if (s === 409) setToast({ show: true, message: "Auditor già assegnato", variant: "danger" });
                      else setToast({ show: true, message: "Errore di connessione", variant: "danger" });
                    } finally {
                      setAssignAuditorBusy(false);
                    }
                  }}
                >
                  Assegna auditor
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      setAssignAuditorBusy(true);
                      await unassignAuditor(id, assignAuditorId);
                      setToast({ show: true, message: "Auditor rimosso", variant: "success" });
                      await loadReport({ silent: true });
                    } catch (e: any) {
                      const s = e?.response?.status;
                      if (s === 403) setToast({ show: true, message: "Operazione non consentita", variant: "danger" });
      ...
      <Card className="info-card mb-3">
        ...
        <Card.Body>{renderPlainWithBreaks(auditor ? "Dato non disponibile per auditor" : (report as any)?.summary ?? "")}</Card.Body>
      </Card>
                      ...
                      <div className="d-flex justify-content-between align-items-center">
                        <div>
                          {isSystemMessage(m) ? (
                            ...
                            <Badge
                              bg={
                                String((m as any).visibility || "INTERNAL").toUpperCase() === "INTERNAL"
                                  ? "secondary"
                                  : "primary"
                              }
                            >
                ...
                <div className="border-top p-3">
                  <Form onSubmit={submitMsg(onMessageSubmit)}>
                    <div className="d-flex align-items-start gap-2">
                      ...
                      />
                      <Button type="submit" disabled={msgBusy} variant="dark">
                        Invia
                      </Button>
                    </div>
                            ...
                            <li className="chat-row" key={m?.id || `m_${i}`}>
                              <div className={"d-flex " + (isAgent ? "justify-content-end" : "justify-content-start")}>
                                <div className="d-inline-block">
                                  {!isAgent && (
                                    <div className="small text-muted fw-semibold ms-1 mb-1">Segnalante</div>
                                  )}
                                  <div
                                    className={
                                      "chat-bubble rounded-3 shadow-sm p-2 " +
                                      (isAgent ? "bg-primary text-white" : "bg-light")
                                    }
                                    style={{ display: "inline-block", maxWidth: "45ch" }}
                                  >
                                    <div className="chat-body">{m?.body}</div>
                                    {time && (
                                      <div
                                        className={"chat-ts small mt-1 " + (isAgent ? "text-white-50" : "text-muted")}
                    ...
                    <Form onSubmit={submitPub(onPublicSubmit)}>
                      <div className="d-flex align-items-start gap-2">
                        <Form.Control
                          ...
                          rows={2}
                          maxLength={5000}
                          placeholder="Scrivi un messaggio pubblico"
                          disabled={pubBusy}
                          {...regPub("body")}
                          style={{ resize: "vertical" }}
                        />
                        <Button type="submit" disabled={pubBusy} variant="dark">
                          Invia
                        </Button>
                      </div>
                    </Form>
                ...
                <div className="mb-3">
                  <Form.Group controlId="fileUpload">
                    <Form.Control
                      type="file"
                      multiple
                      accept=".png,.jpg,.jpeg,.pdf,.txt,.mp3,.wav"
                      disabled={uploading}
                      onChange={(e) => onFilesSelected((e.currentTarget as HTMLInputElement).files)}
                    />
                  </Form.Group>
                  {!!pendingFiles.length && (
                        ...
                        <ListGroup.Item key={`${f.name}-${i}`} className="d-flex justify-content-between align-items-center">
                          <div>
                            ...
                            <small className="text-muted">
                              {(f.size / 1024).toFixed(0)} KB · {f.type || "application/octet-stream"}
                            </small>
                          </div>
                          <div className="d-flex gap-2">
                            <Button size="sm" variant="outline-danger" onClick={() => removePendingAt(i)}>
                              Rimuovi
                            </Button>
                          </div>
                        </ListGroup.Item>
                  ...
                  <div className="d-flex justify-content-between align-items-center mt-2">
                    <small className="text-muted">Max 3 file, 10MB ciascuno, 20 MB totali</small>
                    <div className="d-flex gap-2">
                      <Button
                        size="sm"
                      ...
                      <Button
                        ...
                        variant="dark"
                        disabled={uploading || pendingFiles.length === 0}
                        onClick={savePendingAttachments}
                      >
                        {uploading ? "..." : "Salva"}
                      </Button>
                    </div>
                    ...
                    {attachments.map((a) => (
                      <ListGroup.Item key={a.id} className="d-flex justify-content-between align-items-center">
                        <div>
                          ...
                          <small className="text-muted">
                            {(a as any).mimeType || (a as any)["mime"]} · {((a as any).sizeBytes / 1024).toFixed(0)} KB
                          </small>
                        </div>
                        <div>
                          {auditor ? (
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  const blob = await previewAttachment(id, a.id);
                                  setPreviewBlob(blob);
                                  setPreviewMime(String((a as any).mimeType || ""));
                                  setPreviewOpen(true);
                                } catch (e: any) {
                                  const s = e?.status || e?.response?.status;
                                  if (s === 403)
                                    setToast({ show: true, message: "Operazione non consentita per il tuo ruolo", variant: "danger" });
                                  else if (s === 404) setToast({ show: true, message: "Allegato non disponibile", variant: "danger" });
                                  else setToast({ show: true, message: "Anteprima non disponibile", variant: "danger" });
                                }
                              }}
                            >
                              Anteprima
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  link.href = url;
                                  link.download = (a as any).fileName || "attachment";
              ...
              <Card.Body>
                <Form.Group controlId="myNotes">
                  <Form.Control
                    as="textarea"
                    rows={6}
                    placeholder="Appunti personali sul caso. (visibili solo a te)"
                    value={myNote}
                    onChange={(e) => setMyNote(e.currentTarget.value)}
                    disabled={!canOperate}
                  />
                </Form.Group>
                <div className="d-flex justify-content-between align-items-center mt-2">
                  <small className="text-muted">
                    {myNoteSaving
                      ? "Salvataggio…"
                      : myNoteSavedAt
                      ? `Salvata alle ${new Date(myNoteSavedAt).toLocaleTimeString()}`
                      : "-"}
                    {myNoteError ? ` · ${myNoteError}` : ""}
                  </small>
                  <div className="d-flex gap-2">
                    <Button
                      size="sm"
                      variant="outline-secondary"
                      disabled={!canOperate || (!myNote || myNote.trim() === "") || myNoteSaving}
                      onClick={async () => {
                        try {
                          setMyNoteSaving(true);
                          ...
                          setMyNoteError(e?.response?.data?.message || e?.message || "Errore cancellazione");
                        } finally {
                          setMyNoteSaving(false);
                        }
                      }}
                    ...
                    <Button
                      size="sm"
                      variant="dark"
                      disabled={!canOperate || myNoteSaving}
                      onClick={async () => {
                        try {
                          setMyNoteSaving(true);
                          const res = await putMyNote(id, myNote || "");
                          setMyNoteSavedAt(res?.updatedAt);
                          setMyNoteError(null);
                        } catch (e: any) {
                          setMyNoteError(e?.response?.data?.message || e?.message || "Errore salvataggio");
                        } finally {
                          setMyNoteSaving(false);
                        }
                      }}
      ...
      <ToastContainer position="bottom-end" className="p-3">
        <Toast
          bg={toast.variant === "danger" ? "danger" : toast.variant === "success" ? "success" : "primary"}
